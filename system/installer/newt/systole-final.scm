...
;;; Copyright © 2024 Rafael Palomar <rafael.palomar@ous-research.no>
;;;
...
(define-module (installer newt systole-final)
  #:use-module (installer final)
  #:use-module (installer steps)
...
(define* (run-install-shell locale
  ...
  (clear-screen)
  (newt-suspend)
  (let ((install-ok? (systole-install-system locale #:users users)))
    (newt-resume)
    install-ok?))

(define (run-systole-final-page-install result prev-steps)
  ...
  (let* ((configuration   (systole-format-configuration prev-steps result))
...
(define (dry-run-systole-final-page result prev-steps)
...
(define (run-systole-final-page result prev-steps dry-run?)
  (if dry-run?
      (dry-run-systole-final-page result prev-steps)
      (run-systole-final-page-install result prev-steps)))
