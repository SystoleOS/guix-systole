#!/usr/bin/env bash
# Test that package definitions actually exist and are accessible

set -e

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$REPO_ROOT"

echo "Testing that package definitions exist and are accessible..."

FAILED=0

# Test function to verify a package is defined
test_package() {
    local module="$1"
    local package="$2"

    echo -n "  Testing $package in $module... "

    # Test that we can access the package
    if guix repl -L . <<EOF >/dev/null 2>&1
(use-modules $module)
(unless (defined? '$package)
  (error "Package not defined: $package"))
$package
EOF
    then
        echo "✓"
        return 0
    else
        echo "✗ FAILED"
        FAILED=$((FAILED + 1))
        return 1
    fi
}

# Test VTK packages
test_package "(systole packages vtk)" "vtk-slicer"
test_package "(systole packages vtk)" "vtkaddon"

# Test ITK packages
test_package "(systole packages itk)" "itk-slicer"

# Test Slicer packages
test_package "(systole packages slicer)" "slicer-5.8"
test_package "(systole packages slicer)" "slicer-volumes-5.8"
test_package "(systole packages slicer)" "slicer-terminologies-5.8"
test_package "(systole packages slicer)" "slicer-subjecthierarchy-5.8"
test_package "(systole packages slicer)" "slicer-colors-5.8"
test_package "(systole packages slicer)" "slicer-units-5.8"
test_package "(systole packages slicer)" "slicer-tables-5.8"
test_package "(systole packages slicer)" "slicer-cameras-5.8"
test_package "(systole packages slicer)" "slicer-data-5.8"
test_package "(systole packages slicer)" "slicer-markups-5.8"
test_package "(systole packages slicer)" "slicer-annotations-5.8"
test_package "(systole packages slicer)" "slicer-models-5.8"
test_package "(systole packages slicer)" "slicer-sequences-5.8"
test_package "(systole packages slicer)" "slicer-viewcontrollers-5.8"
test_package "(systole packages slicer)" "slicer-reformat-5.8"
test_package "(systole packages slicer)" "slicer-plots-5.8"
test_package "(systole packages slicer)" "slicer-sceneviews-5.8"
test_package "(systole packages slicer)" "slicer-segmentations-5.8"
test_package "(systole packages slicer)" "slicer-volumerendering-5.8"
test_package "(systole packages slicer)" "slicer-transforms-5.8"
test_package "(systole packages slicer)" "slicer-texts-5.8"
test_package "(systole packages slicer)" "slicer-slicerwelcome-5.8"
test_package "(systole packages slicer)" "slicer-all-5.8"

# Test CTK
test_package "(systole packages ctk)" "ctk"
test_package "(systole packages ctk)" "ctkapplauncher"

# Test Teem
test_package "(systole packages teem)" "teem-slicer"

# Test Math packages
test_package "(systole packages maths)" "netcdf-slicer"

# Test libarchive
test_package "(systole packages libarchive)" "libarchive-slicer"

# Test qrestapi
test_package "(systole packages qrestapi)" "qrestapi"

# Test openigtlink
test_package "(systole packages openigtlink)" "openigtlink"
test_package "(systole packages openigtlink)" "slicer-openigtlink"

# Test ROS 2 helpers
test_package "(systole packages ros2-helpers)" "python-catkin-pkg"
test_package "(systole packages ros2-helpers)" "python-osrf-pycommon"
test_package "(systole packages ros2-helpers)" "eclipse-cyclonedds"
test_package "(systole packages ros2-helpers)" "console-bridge"
test_package "(systole packages ros2-helpers)" "urdfdom-headers"
test_package "(systole packages ros2-helpers)" "urdfdom"

# Test ROS 2 Jazzy packages
test_package "(systole packages ros2 jazzy)" "ros-ament-package-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-core-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-definitions-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-dependencies-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-include-directories-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-libraries-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-link-flags-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-export-targets-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-include-directories-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-libraries-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-target-dependencies-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-version-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-python-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-gtest-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-gmock-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-test-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-gen-version-h-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-pytest-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-vendor-package-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-index-python-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-index-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-domain-coordinator-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ament-cmake-ros-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcutils-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcpputils-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-typesupport-interface-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-runtime-c-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-runtime-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-cli-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-adapter-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-parser-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-pycommon-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-cmake-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-generator-type-description-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-generator-c-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-generator-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-typesupport-introspection-c-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-typesupport-introspection-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-dynamic-typesupport-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rmw-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-typesupport-c-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-typesupport-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-core-generators-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-core-runtime-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-default-generators-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-default-runtime-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-builtin-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-service-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-type-description-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosgraph-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-statistics-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-lifecycle-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-unique-identifier-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-action-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-std-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-std-srvs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-geometry-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-actionlib-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-shape-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-trajectory-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-sensor-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-stereo-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-nav-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-diagnostic-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-visualization-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tinyxml2-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-console-bridge-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-class-loader-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-pluginlib-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-urdf-parser-plugin-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-urdf-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-eigen3-cmake-module-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-orocos-kdl-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-kdl-parser-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-composition-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rclcpp-components-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-message-filters-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-ros-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-eigen-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-eigen-kdl-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-geometry-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-kdl-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-py-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-sensor-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tf2-ros-py-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-diagnostic-updater-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-robot-state-publisher-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-xml-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-yaml-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-testing-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-pytest-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-testing-ament-cmake-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-launch-ros-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-runtime-py-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2cli-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2pkg-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2run-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2node-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2topic-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2service-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2param-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2interface-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2action-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2lifecycle-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2component-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2multicast-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2launch-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-angles-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-example-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-yaml-cpp-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-keyboard-handler-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-shared-queues-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-sqlite3-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-zstd-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-interfaces-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-storage-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-compression-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-compression-zstd-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-storage-sqlite3-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-transport-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-storage-default-plugins-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosbag2-py-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-ros2bag-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-turtlesim-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-object-recognition-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-octomap-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-moveit-msgs-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rmw-dds-common-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-tracetools-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rmw-implementation-cmake-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rmw-cyclonedds-cpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rmw-implementation-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-libyaml-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-yaml-param-parser-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-logging-interface-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-logging-noop-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-lifecycle-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rcl-action-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-libstatistics-collector-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rclcpp-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rclcpp-lifecycle-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rclcpp-action-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-python-cmake-module-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-pybind11-vendor-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rpyutils-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rosidl-generator-py-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-rclpy-jazzy"
test_package "(systole packages ros2 jazzy)" "ros-jazzy"

if [ $FAILED -eq 0 ]; then
    echo "All package definitions verified successfully!"
    exit 0
else
    echo "$FAILED package definition(s) failed verification"
    exit 1
fi
